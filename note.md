...
- async: HTML 파싱과 외부 자바스크립트 파일의 로드가 비동기적으로 동시에 진행된다. 단 파일 로드가 완료되면 바로 실행되어 HTML 파싱이 중단된다.
- defer: async 와 마찬가지로 HTML 파싱과 로드가 비동기적으로 동시에 진행되지만 실행은 HTML 파싱이 완료된 후 실행된다.
...
- 타이머 함수인 setTimemout, setInterval, HTTP 요청, 이벤트 핸들러는 비동기 처리 방식으로 동작한다.
- 이벤트 루프 싱글 스레드인 자바스크립트가 동시에 여러개의 동작을 수행하는 것처럼 보이게 해주는 개념으로 콜 스택이 비어있고 태스크 큐에 대기 중인 함수가 있다면 순차적으로 태스크 큐에 대기 중인 함수를 콜 스택으로 이동시켜 실행한다.
